...
def Object_tracking(Yolo, video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors='', Track_only = []):
    ...
    #for name in file_names:
        ...
        if YOLO_FRAMEWORK == "tf":
            ...
            bboxes,yolodraw = Yolo.detect_image(frame)
        elif YOLO_FRAMEWORK == "trt":
            batched_input = tf.constant(frame)
        ...
        for track in tracker.tracks:
            ...
            cv2.waitKey(0)
            '''
            print("tracking_id:" + str(tracking_id) + ";coordinate:" + str((np.int32(x1 + (x2 - x1) / 2), np.int32(y1 + (y2 - y1) / 2))))
        # draw detection on frame
        image = draw_bbox(original_frame, tracked_bboxes, CLASSES=CLASSES, tracking=True)
        #cv2.imwrite(datetime.datetime.now().strftime('%Y%m%d%H%M%S%f') + ".jpg",image)
        #cv2.imshow("dd",image)
        #cv2.waitKey(0)
        t3 = time.time()
        times.append(t2-t1)
...
yologh = YOLO()
Object_tracking(yologh, video_path, "track.mp4", input_size=YOLO_INPUT_SIZE, show=True, iou_threshold=0.1, rectangle_colors=(255,0,0), Track_only = ["bottle"])
